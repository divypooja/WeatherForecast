...
class PayrollSettingsForm(FlaskForm):
    ...
    standard_working_hours_per_day = FloatField('Standard Working Hours per Day', 
                                               validators=[DataRequired(), NumberRange(min=1, max=24)], 
                                               default=8.0)
    working_days_per_week = IntegerField('Working Days per Week', 
                                        validators=[DataRequired(), NumberRange(min=1, max=7)], 
                                        default=6)
    include_sunday = BooleanField('Include Sunday as Working Day', default=False)
    
    # Overtime Configuration
    overtime_calculation_method = SelectField('Overtime Calculation Method',
                                            choices=[
                                                ('after_standard', 'After Standard Hours'),
                                                ('after_hours', 'After Threshold Hours')
                                            ],
                                            validators=[DataRequired()],
                                            default='after_standard')
    overtime_threshold_hours = FloatField('Overtime Threshold Hours', 
                                         validators=[DataRequired(), NumberRange(min=1, max=24)], 
                                         default=8.0)
    overtime_rate_multiplier = FloatField('Overtime Rate Multiplier', 
                                         validators=[DataRequired(), NumberRange(min=1, max=5)], 
                                         default=1.5,
                                         render_kw={"placeholder": "1.5 = 1.5x regular rate"})
    maximum_overtime_hours_per_day = FloatField('Maximum Overtime Hours per Day', 
                                               validators=[DataRequired(), NumberRange(min=0, max=12)], 
                                               default=4.0)
    
    # Salary Calculation Rules
    salary_calculation_method = SelectField('Salary Calculation Method',
                                          choices=[
    ...
    prorate_partial_days = BooleanField('Count Half Days as 0.5 Days', default=True)
    exclude_weekends_from_calculation = BooleanField('Exclude Weekends from Working Days', default=True)
    auto_calculate_daily_rate = BooleanField('Auto-calculate Daily Rate from Monthly Salary', default=True)
    
    # Leave and Absence Policies
    half_day_hours_threshold = FloatField('Half Day Hours Threshold', 
                                         validators=[DataRequired(), NumberRange(min=1, max=12)], 
                                         default=4.0,
                                         render_kw={"placeholder": "Less than 4 hours = half day"})
    minimum_hours_for_full_day = FloatField('Minimum Hours for Full Day', 
                                           validators=[DataRequired(), NumberRange(min=1, max=12)], 
                                           default=6.0,
                                           render_kw={"placeholder": "At least 6 hours = full day"})
    late_arrival_threshold_minutes = IntegerField('Late Arrival Threshold (Minutes)', 
                                                 validators=[DataRequired(), NumberRange(min=1, max=180)], 
                                                 default=15,
                                                 render_kw={"placeholder": "15 minutes late = Late status"})
    
    # Advance and Deduction Settings
    maximum_advance_percentage = FloatField('Maximum Advance Percentage', 
                                           validators=[DataRequired(), NumberRange(min=1, max=100)], 
                                           default=50.0,
                                           render_kw={"placeholder": "Max 50% of monthly salary"})
    default_advance_repayment_months = IntegerField('Default Advance Repayment Months', 
                                                   validators=[DataRequired(), NumberRange(min=1, max=24)], 
                                                   default=3)
    auto_deduct_advances = BooleanField('Auto-deduct Advances from Salary', default=True)
    
    # Default Rates
    default_overtime_rate_per_hour = FloatField('Default Overtime Rate per Hour (₹)', 
                                               validators=[DataRequired(), NumberRange(min=1)], 
                                               default=100.0)
    default_daily_rate = FloatField('Default Daily Rate (₹)', 
                                   validators=[DataRequired(), NumberRange(min=1)], 
                                   default=500.0)
    
    submit = SubmitField('Save Payroll Settings')
