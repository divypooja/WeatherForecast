...
class AuthenticAccountingIntegration:
    ...
    @staticmethod
    def create_grn_material_receipt_entry(grn):
        """
        Create authentic accounting entry for GRN material receipt
        Dr. Raw Material Inventory, Cr. GRN Clearing Account
        """
        try:
            # Get authentic accounts
            inventory_account = AuthenticAccountingIntegration.get_inventory_account('raw_material')
            clearing_account = AuthenticAccountingIntegration.get_grn_clearing_account()
            
            if not inventory_account:
                logger.error("Raw Material Inventory account not found")
                return None
                
            if not clearing_account:
                logger.error("GRN Clearing Account not found")
                return None
            
            # Calculate total value from GRN line items
            total_value = 0
            for line_item in grn.line_items:
                rate = getattr(line_item, 'rate_per_unit', 0) or getattr(line_item, 'rate', 0) or 0
                total_value += line_item.quantity_received * rate
            
            if total_value <= 0:
                logger.warning(f"GRN {grn.grn_number} has zero value, skipping accounting entry")
                return None
            ...
            entries = [
                {
                    ...
                    'type': 'debit',
                    'amount': total_value,
                    'narration': f'Material received - GRN {grn.grn_number}'
                },
                {
            ...
            # Create voucher using authentic accounting system
            voucher = AuthenticAccountingIntegration.create_simple_voucher(
                voucher_type_code='JOU',  # Journal voucher
                reference_number=grn.grn_number,
                description=f'Material receipt - GRN {grn.grn_number}',
                entries=entries,
            ...
            if voucher:
                logger.info(f"GRN {grn.grn_number} accounting entry created: {voucher.voucher_number}")
                return voucher.voucher_number
            else:
                logger.error(f"Failed to create accounting entry for GRN {grn.grn_number}")
                return None
                
        except Exception as e:
            logger.error(f"Error creating GRN material receipt entry: {str(e)}")
            return None