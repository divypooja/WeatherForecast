...
class PayrollSettings(db.Model):
    ...
    standard_working_hours_per_day = db.Column(db.Float, default=8.0)
    working_days_per_week = db.Column(db.Integer, default=6)  # Monday to Saturday
    include_sunday = db.Column(db.Boolean, default=False)
    
    # Overtime Configuration
    overtime_calculation_method = db.Column(db.String(50), default='after_standard')  # after_standard, after_hours
    overtime_threshold_hours = db.Column(db.Float, default=8.0)  # Hours after which overtime applies
    overtime_rate_multiplier = db.Column(db.Float, default=1.5)  # 1.5x regular rate
    maximum_overtime_hours_per_day = db.Column(db.Float, default=4.0)
    
    # Salary Calculation Rules
    salary_calculation_method = db.Column(db.String(50), default='attendance_based')  # attendance_based, fixed_monthly
    prorate_partial_days = db.Column(db.Boolean, default=True)  # Count half days as 0.5
    exclude_weekends_from_calculation = db.Column(db.Boolean, default=True)
    auto_calculate_daily_rate = db.Column(db.Boolean, default=True)
    
    # Leave and Absence Policies
    half_day_hours_threshold = db.Column(db.Float, default=4.0)  # Less than 4 hours = half day
    minimum_hours_for_full_day = db.Column(db.Float, default=6.0)  # At least 6 hours = full day
    late_arrival_threshold_minutes = db.Column(db.Integer, default=15)  # 15 minutes late = Late status
    
    # Advance and Deduction Settings
    maximum_advance_percentage = db.Column(db.Float, default=50.0)  # Max 50% of monthly salary
    default_advance_repayment_months = db.Column(db.Integer, default=3)
    auto_deduct_advances = db.Column(db.Boolean, default=True)
    
    # Default Rates (can be overridden per employee)
    default_overtime_rate_per_hour = db.Column(db.Float, default=100.0)
    default_daily_rate = db.Column(db.Float, default=500.0)
    ...
    # Relationships
    updater = db.relationship('User', backref='payroll_settings_updates')
    
    @classmethod
    def get_settings(cls):
        """Get payroll settings, create default if none exist"""
        settings = cls.query.first()
        if not settings:
            settings = cls()
            db.session.add(settings)
    ...
    def calculate_working_days_in_period(self, start_date, end_date):
        """Calculate expected working days in a period based on settings"""
        ...
        while current_date <= end_date:
            ...
            if self.include_sunday:
                # All 7 days are working days
                working_days += 1
            else:
                # Sunday (6) is not a working day
                if weekday != 6:
                    working_days += 1
    ...
    def calculate_overtime_amount(self, overtime_hours, regular_hourly_rate=None):
        """Calculate overtime amount based on settings"""
        if not overtime_hours or overtime_hours <= 0:
            return 0.0
        
        # Use provided rate or default
        hourly_rate = regular_hourly_rate or self.default_overtime_rate_per_hour
        
        # Apply overtime multiplier
        overtime_rate = hourly_rate * self.overtime_rate_multiplier
        
        # Cap at maximum overtime hours
        capped_hours = min(overtime_hours, self.maximum_overtime_hours_per_day)
        
        return round(capped_hours * overtime_rate, 2)
    
    def is_overtime_applicable(self, hours_worked):
        """Check if overtime is applicable based on settings"""
        if self.overtime_calculation_method == 'after_standard':
            return hours_worked > self.standard_working_hours_per_day
        elif self.overtime_calculation_method == 'after_hours':
            return hours_worked > self.overtime_threshold_hours
        return False
    
    def calculate_attendance_status(self, hours_worked):
        """Determine attendance status based on hours worked"""
        if hours_worked == 0:
            return 'absent'
        elif hours_worked < self.half_day_hours_threshold:
            return 'half_day'
        elif hours_worked >= self.minimum_hours_for_full_day:
            return 'present'
        else:
            return 'half_day'
